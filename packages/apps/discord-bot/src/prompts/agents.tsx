...
export class AgentPrompt extends PromptElement<AgentPromptProps> {
  ...
  private async getInstructions() {
    ...
    const agentPromptResolver = await PromptRegistry.getPrompt(
      this.props.endpoint,
    );
    if (agentPromptResolver) {
      const resolver =
        this.instantiationService.createInstance(agentPromptResolver);
    ...
    if (msg) {
      firstTurn?.setMetadata(
        new GlobalContextMessageMetadata(
          msg,
          this.instantiationService.invokeFunction(getGlobalContextCacheKey),
        ),
      );
      return msg;
    }
    ...
    return (
      <UserMessage>
        ...
        <Tag name="workspace_info">
          ...
          The view of the workspace structure may be truncated. You can use
          tools to collect more context if needed.
        </Tag>
...
export class AgentUserMessage extends PromptElement<AgentUserMessageProps> {
  ...
  async render(state: void, sizing: PromptSizing) {
    ...
    const isGpt5 =
      this.props.endpoint.family.startsWith('gpt-5') &&
      this.props.endpoint.family !== 'gpt-5-codex';
    const attachmentHint =
      (this.props.endpoint.family === 'gpt-4.1' || isGpt5) &&
      ...
      <>
        <UserMessage>
          ...
          <Tag name="context">
            ...
            {hasTerminalTool && (
              <TerminalStatePromptElement sessionId={this.props.sessionId} />
            )}
            {hasTodoTool && (
              <TodoListContextPrompt sessionId={this.props.sessionId} />
            )}
          </Tag>
          ...
          <Tag name="reminderInstructions">
            ...
            {getEditingReminder(
              hasEditFileTool,
              hasReplaceStringTool,
              modelNeedsStrongReplaceStringHint(this.props.endpoint),
    ...
    return (
      <>
        <Tag name="toolReferences">
          The user attached the following tools to this message. The userRequest
          may refer to them using the tool name with "#". These tools are likely
          ...
          {this.props.modelFamily?.startsWith('gpt-5') === true && (
            <>
              Start by using the most relevant tool attached to this message—the
              user expects you to act with it first.
              <br />
            </>
          )}
        </Tag>
...
class UserShellPrompt extends PromptElement<BasePromptElementProps> {
  ...
  async render(state: void, sizing: PromptSizing) {
    ...
    switch (shellName) {
      ...
      case 'fish': {
        additionalHint =
          ' Note that fish shell does not support heredocs - prefer printf or echo instead.';
        break;
      }
    }
    return (
      <>
        The user's default shell is: "{shellName}"{shellNameHint}. When you
        generate terminal commands, please generate them correctly for this
        shell.{additionalHint}
      </>
    );
  }
}
...
class CurrentEditorContext extends PromptElement<CurrentEditorContextProps> {
  ...
  async render(state: void, sizing: PromptSizing) {
    if (
      !this.configurationService.getConfig(ConfigKey.CurrentEditorAgentContext)
    ) {
      return;
    }
  ...
  private renderActiveTextEditor(activeEditor: TextEditor) {
    ...
    const selectionText =
      selection && !selection.isEmpty ? (
        <>
          The current selection is from line {selection.start.line + 1} to line{' '}
  ...
  private renderActiveNotebookEditor(activeNotebookEditor: NotebookEditor) {
    const altDocument = this.alternativeNotebookContent
      .create(this.alternativeNotebookContent.getFormat(this.props.endpoint))
    ...
    if (
      ...
      const endPosition = altDocument.fromCellPosition(
        endCell,
        new Position(endCell.document.lineCount - 1, lastLine.text.length),
      );
  ...
  async render(state: void, sizing: PromptSizing) {
    ...
    return (
      <Tag name="repoContext">
        Below is the information about the current repository. You can use this
        information when you need to calculate diffs or compare changes with the
        default branch.
        <br />
...
export class AgentTasksInstructions extends PromptElement<AgentTasksInstructionsProps> {
  ...
  render() {
    ...
    return (
      <>
        The following tasks can be executed using the {ToolName.CoreRunTask}{' '}
        ...
        {taskGroups.map(([folder, tasks]) => (
          <Tag
            ...
            attrs={{
              path: this._promptPathRepresentationService.getFilePath(folder),
            }}>
            {tasks.map((t, i) => {
                ...
                <Tag
                  ...
                  {isActive && (
                    <>
                      ...
                      {ToolName.CoreGetTaskOutput} tool to view its output.)
                    </>
                  )}
                </Tag>
              );
...
export function getEditingReminder(
  ...
  if (hasEditFileTool) {
    lines.push(
      <>
        When using the {ToolName.EditFile} tool, avoid repeating existing code,
        instead use a line comment with \`{EXISTING_CODE_MARKER}\` to represent
  ...
  if (hasReplaceStringTool) {
    lines.push(
      <>
        When using the {ToolName.ReplaceString} tool, include 3-5 lines of
        unchanged code before and after the string you want to replace, to make
        it unambiguous which part of the file should be edited.
        <br />
        {hasMultiStringReplace && (
          <>
            ...
            {ToolName.MultiReplaceString} tool rather than sequentially. This
            will greatly improve user's cost and time efficiency leading to a
            better user experience. Do not announce which tool you're using (for
            example, avoid saying "I'll implement all the changes using
  ...
  if (hasEditFileTool && hasReplaceStringTool) {
    ...
    if (useStrongReplaceStringHint) {
      lines.push(
        <>
          You must always try making file edits using the {eitherOr}. NEVER use{' '}
          {ToolName.EditFile} unless told to by the user or by a tool.
        </>,
      );
    } else {
      lines.push(
        <>
          It is much faster to edit using the {eitherOr}. Prefer the {eitherOr}{' '}
          for making edits and only fall back to {ToolName.EditFile} if it
          fails.
        </>,
      );
    }
      ...
      if (
        ...
        return (
          <>
            ...
            However, avoid unnecessary repetition and verbosity. You should be
            concise, but thorough.
            <br />
            You MUST iterate and keep going until the problem is solved.
            <br />
            You have everything you need to resolve this problem. I want you to
            fully solve this autonomously before coming back to me. <br />
            Only terminate your turn when you are sure that the problem is
            solved and all items have been checked off. Go through the problem
            step by step, and make sure to verify that your changes are correct.
            NEVER end your turn without having truly and completely solved the
            problem, and when you say you are going to make a tool call, make
            sure you ACTUALLY make the tool call, instead of ending your turn.
            <br />
            Take your time and think through every step - remember to check your
            solution rigorously and watch out for boundary cases, especially
            with the changes you made. Your solution must be perfect. If not,
            continue working on it. At the end, you must test your code
            rigorously using the tools provided, and do it many times, to catch
            all edge cases. If it is not robust, iterate more and make it
            perfect. Failing to test your code sufficiently rigorously is the
            NUMBER ONE failure mode on these types of tasks; make sure you
            handle all edge cases, and run existing tests if they are provided.{' '}
            <br />
            You MUST plan extensively before each function call, and reflect
            extensively on the outcomes of the previous function calls. DO NOT
            do this entire process by making function calls only, as this can
            impair your ability to solve the problem and think insightfully.
            <br />
            You are a highly capable and autonomous agent, and you can
            definitely solve this problem without needing to ask the user for
            further input.
            <br />
          </>
        );
        ...
        return (
          <>
            ...
            resolved before ending your turn. ONLY stop if solved or genuinely
            blocked.
            <br />
            Take action when possible; the user expects you to do useful work
            ...
            <br />
            Progress cadence: After 3 to 5 tool calls, or when you create/edit
            &gt; ~3 files in a burst, report progress.
            <br />
            Requirements coverage: Read the user's ask in full and think
            carefully. Do not omit a requirement. If something cannot be done
            with available tools, note why briefly and propose a viable
            alternative.
            <br />
          </>
        );
...
function getFileCreationReminder(modelFamily: string | undefined) {
  if (
    ...
    return (
      <>
        Do NOT create a new markdown file to document each change or summarize
        your work unless specifically requested by the user.
        <br />
      </>
    );
  }
...
function getVSCModelReminder(isHiddenModel: boolean) {
  ...
  return (
    <>
      Follow the guidance in &lt;preamble_instructions&gt; from the system
      prompt.
      <br />
      You MUST preface each tool call batch with a brief status update.
      <br />
      Focus on findings and next steps. Vary your openings—avoid repeating
      "I'll" or "I will" consecutively.
      <br />
      When you have a finding, be enthusiastic and specific (2 sentences).
      Otherwise, state your next action only (1 sentence).
      <br />
      Don't over-express your thoughts in preamble, do not use preamble to think
      or reason. This is a strict and strong requirement.
      <br />
    </>
  );
}
...
function getExplanationReminder(
  ...
  return modelFamily?.startsWith('gpt-5') === true ? (
    <>
      Skip filler acknowledgements like "Sounds good" or "Okay, I will…". Open
      with a purposeful one-liner about what you're doing next.
      <br />
      When sharing setup or run steps, present terminal commands in fenced code
      blocks with the correct language tag. Keep commands copyable and on
      separate lines.
      <br />
      Avoid definitive claims about the build or runtime setup unless verified
      from the provided context (or quick tool checks). If uncertain, state
      what's known from attachments and proceed with minimal steps you can adapt
      later.
      <br />
      When you create or edit runnable code, run a test yourself to confirm it
      works; then share optional fenced commands for more advanced runs.
      <br />
      For non-trivial code generation, produce a complete, runnable solution:
      necessary source files, a tiny runner or test/benchmark harness, a minimal
      `README.md`, and updated dependency manifests (e.g., `package.json`,
      `requirements.txt`, `pyproject.toml`). Offer quick "try it" commands and
      optional platform-specific speed-ups when relevant.
      <br />
      Your goal is to act like a pair programmer: be friendly and helpful. If
      you can do more, do more. Be proactive with your solutions, think about
      what the user needs and what they want, and implement it proactively.
      <br />
      <Tag name="importantReminders">
        Before starting a task, review and follow the guidance in
        &lt;responseModeHints&gt;, &lt;engineeringMindsetHints&gt;, and
        &lt;requirementsUnderstanding&gt;.
        <br />
        {!isGpt5Mini && (
          <>
            Start your response with a brief acknowledgement, followed by a
            concise high-level plan outlining your approach.
            <br />
          </>
        )}
        Do NOT volunteer your model name unless the user explicitly asks you
        about it. <br />
        {hasTodoTool && (
          <>
            You MUST use the todo list tool to plan and track your progress.
            NEVER skip this step, and START with this step whenever the task is
            multi-step. This is essential for maintaining visibility and proper
        ...
        {!hasTodoTool && (
          <>
            Break down the request into clear, actionable steps and present them
            at the beginning of your response before proceeding with
            implementation. This helps maintain visibility and ensures all
            requirements are addressed systematically.
            <br />
          </>
        )}
        When referring to a filename or symbol in the user's workspace, wrap it
        in backticks.
        <br />
      </Tag>
    </>
  ...
  async render(state: void, sizing: PromptSizing) {
    ...
    for (const event of events) {
      if (event.eventKind === ChatRequestEditedFileEventKind.Undo) {
        ...
        event.eventKind === ChatRequestEditedFileEventKind.UserModification
      ) {
        const fp = this.promptPathRepresentationService.getFilePath(event.uri);
        if (!seenMod.has(fp)) {
          seenMod.add(fp);
    ...
    return (
      <>
        ...
        {sections.join('\n')}
        <br />
        So be sure to check the current file contents before making any new
        edits.
      </>
    );
  }
